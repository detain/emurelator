...
* create public website on github [https://github.com/detain/emurelator.github.io](detain/emurelator.github.io)
* add wiki entries
...
tsessrun 'cd /root/cpaneldirect && git pull --all && ln -fs /root/cpaneldirect/emurelator.phar /usr/local/bin/emurelator && php emurelator.phar bash --bind emurelator.phar --program emurelator.phar > /etc/bash_completion.d/emurelator_completion && chmod +x /etc/bash_completion.d/emurelator_completion && if [ -e /etc/apt ]; then apt-get update &&  apt-get autoremove -y --purge && apt-get dist-upgrade -y && apt-get autoremove -y --purge && apt-get clean; else yum update -y --skip-broken; fi'
tsessrun 'cd /root/cpaneldirect && git pull --all && ln -fs /root/cpaneldirect/emurelator.phar /usr/local/bin/emurelator && php emurelator.phar bash --bind emurelator.phar --program emurelator.phar > /etc/bash_completion.d/emurelator_completion && chmod +x /etc/bash_completion.d/emurelator_completion && if [ -e /etc/apt ]; then apt-get update &&  apt-get autoremove -y --purge && apt-get dist-upgrade -y && apt-get autoremove -y --purge && apt-get clean; else yum update -y --skip-broken; fi && if [ "$(php -v|head -n 1|cut -c5)" = 7 ]; then exit; fi;'
```
